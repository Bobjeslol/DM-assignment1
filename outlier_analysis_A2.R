...
# Read the dataset
data <- read.csv("training_set_VU_DM.csv",header=TRUE, na.strings = c("NULL", "", NULL))
usage_vars <- colnames(data)
columns_to_remove <- c("srch_id", "site_id", "visitor_location_country_id", "prop_country_id", "prop_id", "date_time", "srch_destination_id", "prop_brand_bool", "promotion_flag", "srch_saturday_night_bool", "random_bool", "click_bool", "booking_bool")
usage_vars <- setdiff(usage_vars, columns_to_remove)
...
for (var in usage_vars) {
  ...
  before_fit <- fit_best_distribution(var_data, var, "before")
  distribution_fit_summary <- rbind(distribution_fit_summary, before_fit)
  
  # Create distribution plots for the raw data
  create_distribution_plots(var_data, var, before_fit, "before")
  
  # Step 2: Perform LOF outlier detection
  cat("Performing LOF outlier detection...\n")
  k_value <- min(20, max(10, floor(length(var_data) * 0.005)))
  value_matrix <- matrix(var_data, ncol = 1)
  ...
  cat("Detected", length(outliers), "outliers out of", length(var_data), "data points\n")
  ...
  outlier_data_list[[var]] <- data.frame(
    srch_id = data$srch_id[outliers],
    site_id = data$site_id[outliers],
    visitor_location_country_id = data$visitor_location_country_id[outliers],
    prop_country_id = data$prop_country_id[outliers],
    prop_id = data$prop_id[outliers],
    srch_destination_id = data$srch_destination_id[outliers],
    date_time = data$date_time[outliers],
    value = var_data[outliers],
  ...
  after_fit <- fit_best_distribution(var_data_clean, var, "after")
  distribution_fit_summary <- rbind(distribution_fit_summary, after_fit)
  
  # Create distribution plots for the cleaned data
  create_distribution_plots(var_data_clean, var, after_fit, "after")
  
  # Step 4: Compare distributions before and after
  create_comparison_plot(var_data, var_data_clean, var, before_fit, after_fit)
  
  # Calculate statistical summaries before and after
  mean_before <- mean(var_data)
  mean_after <- mean(var_data_clean)
  mean_outliers <- mean(var_data[outliers])
  ...
  lof_outlier_summary <- rbind(
    ...
    data.frame(
      ...
      outlier_percent = round(100 * length(outliers) / length(var_data), 2),
...
for (var in names(outlier_data_list)) {
  ...
  # Build key from outliers info (assuming it contains srch_id + time)
  outlier_keys <- paste(outliers_info$srch_id, outliers_info$site_id, outliers_info$visitor_location_country_id, outliers_info$prop_country_id, outliers_info$prop_id, outliers_info$srch_destination_id, outliers_info$date_time)
  
  # Build key for all data (ensure columns exist)
  var_data_all$key <- paste(var_data_all$srch_id,
                            var_data_all$site_id,
                            var_data_all$visitor_location_country_id,
                            var_data_all$prop_country_id,
                            var_data_all$prop_id,
                            var_data_all$srch_destination_id,
                            var_data_all$date_time)
  
  # Remove outlier rows
  var_data_clean <- var_data_all[!var_data_all$key %in% outlier_keys, ]
  var_data_clean$key <- NULL
